...
const String _defaultCoverImage = 'lib/assets/Fourth_Wing_Cover_Art.jpeg';
...
class _MyListingsScreenState extends ConsumerState<MyListingsScreen>
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: _getStatusColor(book.swapStatus)
                                                .withValues(alpha: 0.1),
                                            borderRadius: BorderRadius.circular(8),
                                            border: Border.all(
                                              color: _getStatusColor(book.swapStatus)
                              ...
                              children: [
                                IconButton(
                                  ...
                                  onPressed: () => _handleSwapOffer(
                                      context, offer.id, 'Accepted'),
                                ),
                                IconButton(
                                  icon: const Icon(Icons.close, color: Colors.red),
                                  onPressed: () => _handleSwapOffer(
                                      context, offer.id, 'Rejected'),
                                ),
                              ],
                            )
                          : Text(
                  ...
                  : () async {
                      ...
                      try {
                        ...
                        if (selectedImage != null || selectedImageBytes != null) {
                          if (kIsWeb && selectedImageBytes != null) {
                            // For web, we need to upload from bytes
                            // Create a temporary file or upload directly from bytes
                            imageUrl = await storageService.uploadBookCoverFromBytes(
                                selectedImageBytes!);
  ...
  Future<void> _handleSwapOffer(
      BuildContext context, String offerId, String status) async {
    try {
      final firestoreService = ref.read(firestoreServiceProvider);
      await firestoreService.updateSwapOfferStatus(offerId, status);
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Swap offer $status')),
        );
      }
    } catch (e) {
      if (context.mounted) {
